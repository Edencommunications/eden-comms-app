...
- Hardened (Aug 2026): staff_meta writes are admin-only. The admin UI saves via api-server POST /staff/meta (admin JWT verified, service key upsert, same-org + connect_coach validation); direct PostgREST writes of `staff_meta:%` keys are blocked for non-admins by the admin_settings RLS script `database-updates/2026-08-18-staff-meta-admin-only.sql` (mirrored in rls_phase2.sql — user must paste-and-run it in Supabase). SELECT stays org-scoped so staff sessions still read their own row for tab gating; the `staff-meta-changed` broadcast nudge is unchanged.
