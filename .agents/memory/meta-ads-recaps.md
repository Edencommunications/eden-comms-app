...
- Per-org time zone (cfg `tz`, default America/Chicago) + `hour_local` post hour, same model as GHL KPI: legacy UTC `hour` honored until an hourLocal is saved (saving hourLocal deletes `hour`). Dueness (daily/weekly Monday/monthly 1st) and dedupe markers use the ORG-local calendar; recap data windows stay UTC-date based (Meta insights use the ad account's own tz anyway).
