...
- Lesson-order mode: admin_settings key `course_mode:<courseId>` under the course's OWNING org (admin_settings.company_id is NOT NULL — no org-neutral rows possible), value "sequential". Read via GET /course-modes (any user; service key, so cross-org Eden courses work) — direct RLS reads fail for shared courses. Canonical lesson order everywhere: (section_id, sort_order, id). Server enforces order in /dba/progress; Week5 lock is client-side only (Week5 writes course_progress directly with legacy module_id strings, DBA uses row UUIDs — two semantics in one table, don't mix).
...
- DM pairing (user-confirmed): staff-grade side (coach/org admin/delegated leader) may OPEN a 1v1 with anyone; the other side then replies inside that thread regardless of own DM access. Plain member↔member still needs BOTH sides DM-capable (tier gate). Members w/o DM access cannot initiate with anyone, including the coach.
