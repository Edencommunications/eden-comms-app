...
- Duplicate-account guard: user_profiles has NO unique email index and never can (schema frozen). The only real DB-level uniqueness is auth.users' unique login email, so every new-account flow must provision the login FIRST via the shared duplicate-rejecting primitive (never raw check-then-insert, never idempotent "existed = success"); only logins older than ~10 min with no profile count as repairable orphans — a fresh one is a concurrent creation.
