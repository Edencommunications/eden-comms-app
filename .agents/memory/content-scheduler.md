...
- TikTok photos/carousels use PULL_FROM_URL (`/v2/post/publish/content/init/`, media_type PHOTO) — TikTok fetches images itself and requires the URL prefix be a verified URL property. Images relay through a public api-server route on the app's own domain (`public_base` captured at app-save), which also serves the admin-pasted TikTok verification file. A PULL_FROM_URL init alone can publish → any non-definitive init failure is ambiguous (structured TikTok error responses are marked `definitive` and safe to surface).
