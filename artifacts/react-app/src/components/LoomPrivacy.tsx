  ...
  return (
    <span onClick={e => { e.stopPropagation(); if (key) loomToggleShown(key) }}
      ...
      style={{ filter: vis ? 'none' : 'blur(6px)', cursor: 'pointer',
        ...
        outline: vis ? '1px dashed #ffa60066' : 'none', ...style }}>
      {children}
    </span>
  )
}
    ...
    <div style={{ position:'relative' }}>
      <button onClick={() => setOpen(v => !v)}
        ...
        style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2,
          background: open ? '#ffa60022' : 'transparent',
          border:`1.5px solid ${open ? '#ffa600' : '#2a2a2a'}`,
          borderRadius:8, padding:'4px 8px', cursor:'pointer' }}>
        <span style={{ fontSize:15 }}>👁</span>
        {!isMobile && (
          <span style={{ fontSize:8, fontWeight:700, letterSpacing:.6, textTransform:'uppercase',
            color: open ? '#ffa600' : '#888' }}>Visible</span>
        )}
      </button>
      {open && (
        <div style={{ position:'absolute', top:'calc(100% + 8px)', right:0, width:250, maxHeight:320,
          overflowY:'auto', background:'#1a1a1a', border:'1px solid #2a2a2a', borderRadius:12,
          ...
          <div style={{ fontSize:10, color:'#888', padding:'0 6px 8px', lineHeight:1.5 }}>
            Check the names to keep readable, then close this menu before recording.
          </div>
          {list.length === 0 && (
            <div style={{ fontSize:11, color:'#888', padding:'6px 6px 8px', lineHeight:1.5 }}>
          ...
          {list.map(k => (
            <label key={k} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 6px',
              borderRadius:8, cursor:'pointer', fontSize:12, color:'#fff', fontWeight:600 }}>
              <input type="checkbox" checked={shown().has(k)} onChange={() => loomToggleShown(k)}
                style={{ accentColor:'#ffa600', cursor:'pointer' }}/>
